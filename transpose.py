...
void transpose_v4(const float* __restrict__ a, float* __restrict__ out, int M, int N) {
    ...
    int tx = threadIdx.x, ty = threadIdx.y;
    for (int row = ty; row < TILE; row += blockDim.y) {
        int src_row = blockIdx.y * TILE + row, src_col = blockIdx.x * TILE + tx;
        if (src_row < M && src_col < N) tile[row][tx] = a[src_row * N + src_col];
    }
    __syncthreads();
    for (int row = ty; row < TILE; row += blockDim.y) {
        int dst_row = blockIdx.x * TILE + row, dst_col = blockIdx.y * TILE + tx;
...
KERNELS = {
    "v1:naive": {
        "kernel": _compile(v1_src, "transpose_v1"),
        "block": (TILE, TILE),
        "tile": (TILE, TILE),
    },
    "v2:shmem": {
        "kernel": _compile(v2_src, "transpose_v2"),
        "block": (TILE, TILE),
        "tile": (TILE, TILE),
    },
    "v3:padding": {
        "kernel": _compile(v3_src, "transpose_v3"),
        "block": (TILE, TILE),
        "tile": (TILE, TILE),
    },
    "v4:128t": {
        "kernel": _compile(v4_src, "transpose_v4"),
        "block": (TILE, 4),
        "tile": (TILE, TILE),
    },
}
